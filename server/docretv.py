...
def create_document_retriever(config,event_loop):
    ...
    if class_name in ['CMKBElasticSearchRetriever']:
        return _cls.from_config(kwargs['config'])
    elif class_name in ['GoogleSearchRetriever']:
...
class CMKBElasticSearchRetriever():
    ...
    def from_config(cls,config):
        elk_db =  CMKBElasticDB(**config["elk_db"])
        keyword_dict = set([l.rstrip() for l in open(config["word_dict"],'r',encoding='utf-8')])
        return CMKBElasticSearchRetriever(elk_db,config["k"],keyword_dict)
    def __init__(self,elk_db,k,word_dict):
        self.k = k
        self.elk_db = elk_db
    ...
    def search_elk(self,question):
        ...
        docs = self.elk_db.retrieve_library_doc(keywords,size=self.k)
