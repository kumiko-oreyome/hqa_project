...
from multidoc.dataset.dureader import load_dataset,DureaderExample,filter_no_answer_doc_examples,flatten_examples
from multidoc.dataset.drcd import load_drcd_examples
...
def gold_span_preprocessing(source,max_q_len,max_seq_length):
...
def train():
    config = JsonConfig('./models/reader/bert_reader/drcd/config.json')
    ...
    preprocess_funcs = [lambda x: gold_span_preprocessing(x,max_q_len,max_seq_len)]
    ...
    for epcoh in range(epoch_num):
        ...
        print('evaluate')
        eval_examples = load_drcd_examples(dev_paths,'answer_doc')
        eval_examples = filter_no_answer_doc_examples(eval_examples)
        eval_examples = flatten_examples(eval_examples,['question_id','question','answers'],[])  
        result = evaluator.evaluate_reader_on_examples(eval_examples,reader)
        print(result)
